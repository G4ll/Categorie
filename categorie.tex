...
allo specifico isomorfismo di rappresentabilità a sinistra $ \Phi\colon\text{Coni}(F, \, \bullet \, ) \simeq \Hom_\CC(X, \, \bullet \,) $, corrisponde infatti un cono $ F \Rightarrow X $ a destra.\\

Mostriamo che $ X $ è l'oggetto iniziale nella categoria dei coni sotto $ F $: vogliamo mostrare che, preso un cono $ F \Rightarrow Z $ sotto $ F $, esiste un'unica trasfromazione naturale $ \varphi $ che fa commutare il diagramma
\[\begin{tikzcd}[ampersand replacement = \&]
F
\rar[Rightarrow]
\dar[Rightarrow] \&
X \arrow[dl, Rightarrow, "\varphi"]\\
Z 
\end{tikzcd}  \]

Prendiamo un cono $ \gamma \colon F \Rightarrow Z $ sotto $ F $: questo è un elemento di Coni$(F, \, Z)$ e corrisponde pertanto a un morfismo $ \Phi_Z(\gamma) \colon X \to Z $. Dalla naturalità del Lemma di Yoneda otteniamo un morfismo associato $ C(f)\colon\text{Coni}(F, \, X) \to \text{Coni}(F,\, Z) $ che, in particolare, produce un morfismo dal cono limite $ X $ al cono $ Y $: 
\[\begin{tikzcd}[ampersand replacement = \&]
\Hom_{\Fun(\CC, \, \Set)}(\Hom_\CC(X, \, \bullet\,), \, \text{Coni}(F, \, \bullet \, ))
\rar
\dar["(f^*)^*"] \&
\text{Coni}(F, \, X )
\dar["C(f)"] \\
\Hom_{\Fun(\CC, \, \Set)}(\Hom_\CC(Z, \, \bullet\,), \, \text{Coni}(F, \, \bullet \, ))
\rar \&
\text{Coni}(F, \, Z ).
\end{tikzcd}  \]
